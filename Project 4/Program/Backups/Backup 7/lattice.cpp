...
lattice::lattice(const unsigned n, const double t)
{
    _T = t;
    _dim = n;
    double beta = 1. / (K * _T);
...
void lattice::montecarlo(const unsigned long mc_cycles, const std::string folder)
{
    double E;                               //  energy
    ...
    std::ofstream out_results(folder + "results");
    ...
    for(long cycle = 0; cycle < mc_cycles; cycle++)
    {
        ...
        //  cycle  E(cycle)  |M|(cycle)
        _output(out_values, cycle, E, M_abs);   //  output values as functions of the number of cycles
    }
    ...
    // T  <E>(T)  <|M|>(T)  variance(E)  Cv  Khi, last values only
    _output(out_results, averages, mc_cycles);
...
void lattice::montecarlo(const unsigned long mc_cycles, const double final_temp, const double temp_step, const std::string folder)
{
    ...
    double khi;                             //  susceptibility
    double E_variance;                      //  variance of E
    long accepted_configs = 0;              //  accepted metropolisconfigurations
    unsigned* position = new unsigned[2];   //  position of the randomly flipped spin
    double* averages = new double[4];       //  0 : <E>, 1 : <E> * <E>, 2 : |M|, 3 : |M| * |M|
   
    std::ofstream out_values(folder + "expected values with time");
    std::ofstream out_results(folder + "results");
    ...
    while(_T < final_temp)
    {
        ...
        //  T  <E>(T)  <|M|>(T)  Cv  khi  accepted_configs(T)
        _output(out_values, averages, mc_cycles, Cv, khi, accepted_configs);    //  output values as functions of temp at each temp
        _T += temp_step;
        _update_precalc();
    }
...
void lattice::_output(std::ofstream& out_values, const unsigned long cycle, const double E, const double M_abs)
{
    if(cycle % 5000000 == 0)
    {
        out_values << cycle << setw(15);
        out_values << E << setw(15);
        out_values << abs(M_abs) << setw(15);
        out_values << std::endl;
    }
}
...
void lattice::_output(std::ofstream& out_results, const double* averages, const unsigned long mc_cycles)
{
    ...
    double E_variance = expect_E2 - expect_E * expect_E;
    double Cv = E_variance * (_precalc[6] / _T);
    double khi = (expect_M2 - expect_M * expect_M) * _precalc[6];
    
    out_results << _dim << "x" << _dim << " lattice" << std::endl;
    out_results << "Temp: " << _T << std::endl;
    out_results << "Monte Carlo cycles per temp-step: " << mc_cycles << std::endl;
    
    out_results << std::endl;
    out_results << "Temp" << setw(15) << "<E>" << setw(15);
    out_results << "<|M|>" << setw(15) << "variance(E)" << setw(15) << Cv << setw(15) << "khi" << std::endl;
    out_results << setw(15) << setprecision(8) << _T;
    out_results << setw(15) << setprecision(8) << expect_E;
    out_results << setw(15) << setprecision(8) << expect_M;
    out_results << setw(15) << setprecision(8) << E_variance;
    out_results << setw(15) << setprecision(8) << Cv;
    out_results << setw(15) << setprecision(8) << khi << endl;
}
